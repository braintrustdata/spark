...
const PRODUCTION_TOKEN_MESSAGE =
  "Add the BRAINTRUST_API_KEY token from your local Braintrust token files to your deployment platform's environment variables so tracing works in production.";
    ...
    expect(
      events.some(
        (event) =>
          ...
          event.includes(
            "Run your app locally and invoke AI functionality to test whether tracing is working.",
          ) &&
    ...
    expect(
      events.some(
        (event) =>
          ...
          event.includes("Env file: .env.braintrust"),
      ),
    ).toBe(true);
    ...
    expect(
      events.some(
        (event) =>
          ...
          event.includes("Braintrust instrumentation completed?"),
      ),
    ).toBe(true);
    expect(events).not.toContain(
